...
esat::Vec3 g_figurita[6] {
    {0.0f, 0.0f, 1.0f}, // esta ultima es la goat, homogenea, se repite por cada par de puntos, por eso hacemos Vec3
    {0.0f, 0.75f, 1.0f},
    {0.125f, 1.0f, 1.0f},
    {0.25f, 0.75f, 1.0f},
    {0.25f, 0.0f, 1.0f},
...
esat::Mat3 UpdateFigurita(esat::Vec2 scale, float angle, esat::Vec2 whereMove) {
    ...
    m = esat::Mat3Multiply(esat::Mat3Translate(-0.125f, 0.0f), m);
...
int esat::main(int argc, char **argv) {
    ...
    while(esat::WindowIsOpened() && !esat::IsSpecialKeyDown(esat::kSpecialKey_Escape)) {
        ...
        for (int i = 0; i < 8; i += 2) {
            ...
            esat::Mat3 matriz = UpdateFigurita({40.0f, 40.0f}, angle * i , {0.0f, 40.0f});
        ...
        for (int i = 1; i < 8; i += 2) {
            ...
            esat::Mat3 matriz = UpdateFigurita({40.0f, 40.0f}, angle2 * i, {0.0f, -80.0f});
            DrawFigurita(matriz, 6);
        }
